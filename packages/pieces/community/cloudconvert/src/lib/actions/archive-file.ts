...
const archiveFileProps = () => ({
  ...
  files: Property.Array({
    ...
    properties: {
      ...
      stored_file_id: Property.ShortText({
        ...
        description: 'ID of a previously stored file in JRNYFLW to archive',
        required: false,
      }),
  ...
  store_file: Property.Checkbox({
    ...
    description: 'Download and store the created archive in JRNYFLW',
    required: false,
    defaultValue: true,
  }),
});
            ...
            files.forEach((file: any, index: number) => {
                ...
                if (import_method === 'url') {
                    ...
                    if (!context.server?.apiUrl) {
                        throw new Error('Server API URL is not available. Please check your JRNYFLW server configuration.');
                    }
